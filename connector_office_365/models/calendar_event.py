...
class CalendarEvent(models.Model):
    ...
    def _office_365_to_event(self, event):
        ...
        start = parser.parse(event['start']['dateTime']).strftime("%Y-%m-%d %H:%M:%S")
        stop = parser.parse(event['end']['dateTime']).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if event['isAllDay']:
            ...
            stop = datetime.datetime.strptime(stop,"%Y-%m-%d %H:%M:%S").date()
            #_logger.debug("\n\n\n START: %s --- %s , STOP: %s ---- %s \n\n\n",start,type(start),stop,type(stop))
            stop -= timedelta(days=1)
    ...
    def _office_365_push_create(self):
        ...
        _logger.error('self.name %s', self.name)
